...
public class CombatScript : MonoBehaviour
{
    ...
    [SerializeField] AudioSubject audioSubject; //calls the audio observer for sound trigger
    ...
    public void Attack(EnemyScript target, float distance)
    {
        ...
        if (distance < 15)
        {
            ...
            //see what animation attack type is playing. See the animator for the paramter name to the animation eg Airkick = Crecent Kick
           Debug.Log("attack type " + attackString); 
        }
        else
        {
            lockedTarget = null;
        ...
        if(distance > 5)
        {
            LeapAttackSound(true);//play the woosh sound
        }
        
    }
    //use this to bool a sound parameter if if true sound is playing.
     
    public void LeapAttackSound(bool isAttackingSound )
    {
        if(isAttackingSound)
        {
            audioSubject.leapAttackStart();
    ...
    void AttackType(string attackTrigger, float cooldown, EnemyScript target, float movementDuration)
    {
        ...
        {
            audioSubject.LastSoundhit(); //set sounds for the last blow. 
            Time.timeScale = .5f;
